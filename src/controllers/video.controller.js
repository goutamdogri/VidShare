...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  async function updateVideoWithThumbnail(videoId, title, description) {
    try {
      const filePath = "./public/temp/url.txt"; // Replace with your actual path
      ...
      // Use the updatedVideo object here
      return updatedVideo; // Or perform other actions with it
    } catch (error) {
      console.error("Error reading file or updating video:", error);
      throw error; // Or handle the error differently (e.g., return an error response)
    }
  }

  const updatedVideo = updateVideoWithThumbnail(video._id, title, description);

  // let thumbnail = "";
  // await fs.readFile("./public/temp/url.txt", "utf8", (err, data) => {
  //   if (err) throw err;
  //   console.log("247", data);
  //   thumbnail = data;
  //   console.log("249", thumbnail);
...
const addViewCount = asyncHandler(async (req, res) => {
  ...
  if (!watchHistoryUpdate)
    throw new ApiError(
      500,
      "video does not added to watchHistory"
    );
